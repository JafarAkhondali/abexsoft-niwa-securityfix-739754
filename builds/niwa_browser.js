...
niwa.Browser = function(app_ui_klass, world_worker) {
    var that = this;
    
    this.userInterface = new niwa.UserInterface(app_ui_klass);
    if (world_worker)
        this.worldWorker = world_worker;
    else
...
niwa.UserInterface.prototype.update = function(models) {
    for (key in models) {
	    ...
	    if (this.actors[key] === undefined) {
            ...
            if (model.curAnimLabel){
                if (this.actors[key].curAnimLabel != model.curAnimLabel ||
                    this.actors[key].curAnimFps != model.curAnimFps) {
                    if (this.actors[key].playAnimation(model.curAnimLabel, model.curAnimFps)) {
                        this.actors[key].curAnimLabel = model.curAnimLabel;
                        this.actors[key].curAnimFps = model.curAnimFps;
                    }
                }
            }
...
(function (exports) {
    exports.JsonMorphAnimMeshActorInfo = JsonMorphAnimMeshActorInfo;
    
    // The instance of this class is sent throght network. 
    function JsonMorphAnimMeshActorInfo(model) {
        this.model = model;

        // mesh scale
        this.scale = new exports.Vector3D(1.0, 1.0, 1.0);
        ...
        // for visual
        this.textureName = null;
        this.animLabels = null;  // label[0]: start, label[1]: end, label[2]: fps
        
        this.type = exports.JsonMorphAnimMeshActorInfo.prototype.type;
    };
    JsonMorphAnimMeshActorInfo.prototype.type = "json_morph_anim_mesh";
    ...
    loader.load(actorInfo.model, 
                function ( geometry, materials ) {
                    ...
                    if (actorInfo.animLabels){
                        ...
                        // for test
                        //that.mesh.playAnimation("walk", actorInfo.animLabels["walk"][2]); 
                    }
